...
const Header = () => {
  ...
  return (
    <nav className="flex justify-between px-10 items-center py-3 bg-neutral-100 shadow">
      <Link onClick={closeList} to="/" className="text-lg font-bold text-black">
        Karo Abhayaas
      </Link>
      <div>
        <div className={`absolute md:relative rounded-s-lg rounded-br-lg md:rounded-none z-10 bg-stone-200 p-5 md:p-0  md:bg-neutral-100 right-4 top-10 md:right-auto md:top-auto  flex-col md:flex-row flex justify-evenly items-start gap-2 md:gap-5 md:items-center lg:gap-12 text-black ${showList ? "block" : "hidden md:flex"}`}>
          <NavLink onClick={closeList} to="/login" className={({isActive})=> isActive ? "text-orange-300" : "text-black" }>
            <Button text={"JOIN Us"} />
          </NavLink>
          <NavLink onClick={closeList} to="/classes" className={({isActive})=> isActive ? "text-base font-thin text-orange-300" : "text-base font-thin"}>
            Classes
          </NavLink>
          <NavLink onClick={closeList} to="/products" className={({isActive})=> isActive ? "text-base font-thin text-orange-300" : "text-base font-thin"}>
            Products
          </NavLink>
          <NavLink onClick={closeList} to="/about" className={({isActive})=> isActive ? "text-base font-thin text-orange-300" : "text-base font-thin"}>
            About Us
          </NavLink>
          <NavLink onClick={closeList} to="/contact" className={({isActive})=> isActive ? "text-orange-300" : "text-black" }>
            <IoMdContact className="h-5 w-5 flex-shrink-0" />
          </NavLink>
          <NavLink onClick={closeList} to="/cart" className={({isActive})=> isActive ? "text-orange-300 relative" : "text-black relative" }>
